    ...
    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4  min-h-screen pb-10 bg-gray-100 px-4">
      <div className="md:col-span-2 md:mr-3 lg:col-span-3 lg:mr-4 flex flex-col bg-white mt-6 p-4">
        <div className="relative border-b border-gray-200 pb-5">
          ...
          <p className="text-xs sm:text-sm text-blue-700">Deselect all items</p>
        ...
        <div className="border-b border-gray-200 my-4 ">
          {totalQty === 0 ? (
            <div className="mb-4">
              ...
              <Link href="/">
                <a className="text-xs text-blue-800">See recommendations</a>
              </Link>
            </div>
          ) : (
            <CheckoutProductList />
      ...
      <div className="h-36 flex flex-col bg-white p-3 mt-3 whitespace-nowrap">
        <p className="text-base sm:text-base md:text-base mr-12">
          Subtotal ({totalQty === 1 ? `${totalQty} item` : `${totalQty} items`}
          ):{" "}
          <span className="text-sm sm:text-base font-medium">${totalAmt}</span>
        </p>
